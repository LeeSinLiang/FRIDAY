...
**Where the walls are.** For a fixture room, its own width and depth. For a prepared room (one with `spatial`), **the edges of the free floor, not the modelled shell** (`wallBounds` in `boundary.ts`). The shell of the Cg Arch interior is 1158 × 844 cm but furniture may only stand in a 334 × 580 cm rectangle; measured to the shell, "5 feet from any wall" passed with 2,580 positions for the wrong reason. Measured correctly it does not fit, and says `needs 373 cm of width, this room has 334 cm`. Two deliberate approximations for now: the bounding box of the free areas is used, so an L-shaped floor is its enclosing rectangle for wall clauses (placement itself stays exact, that is `validatePlacement`'s job); and every edge counts as a wall, although the Cg Arch x = 787 cm edge is really an opening into the adjoining space. `spatial.json` carries no wall metadata to say otherwise.
