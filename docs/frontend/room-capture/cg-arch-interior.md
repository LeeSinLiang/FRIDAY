...
Only the living-room zone x=787..1121, z=180..760 cm is enabled for furniture and walking. Other rooms are visual context. Preparation checks 7,839 vertical rays at 5 cm spacing across the source living-room rectangle for clear floor support, and fails if they encounter an obstruction. This sampling is a fixture check, not a general collision algorithm or proof against all thin/overhead obstacles. Runtime validation uses the conservative rectangular floor map and furniture footprints. The 5 cm placement grid is unrelated to real-world scan accuracy.

The default camera is at (1100, 121, 735) cm, yaw 0.898638 rad, pitch -0.023463 rad, with a 57-degree vertical field of view. It follows the source camera direction and low architectural eye height while moving 26 cm inward for the 20 cm navigation clearance. Its framing still needs browser comparison after the lighting bake; matching a source camera direction does not guarantee matching the catalogue image crop. The expanded living-room rectangle was checked against evaluated source geometry at 5 cm intervals from 2.4 m down to the floor, with zero obstructions in 7,839 rays. This permits the source-like rear viewing position without enabling the adjoining rooms.
