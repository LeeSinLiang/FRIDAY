...
The room index and preview deck read the existing build-provided room catalogue. `shared/public-rooms.json` lists Haussmann apartment first, followed by the studio and London skyscraper. The fetched public release removes the separately licensed Cg Arch room from the gallery; its explicit local route remains available. Haussmann is initially selected and starts downloading in the background when its local visual asset is missing. The large view is the existing room preview, including any editor controls captured in that image; it is not a generated architectural substitute. Other unpackaged rooms remain disabled. Haussmann has a verified browser-download fallback, real progress and retry. A failed preview displays a fallback without changing room availability.

The Haussmann preview at `frontend/public/room-previews/haussmann-apartment.png` is a 960 × 640 photographic capture of the empty Gaussian apartment, rendered with zero furniture instances. Room source: [HAUSSMANN APARTMENT by Stéphane Agullo](https://superspl.at/scene/4de797f4), [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/); the preview contains only the room. The room editor retains the source attribution. The gallery list is read when Vite starts; restart Vite or rebuild after changing it.

See [automatic Haussmann download](../room-capture/haussmann-apartment.md#automatic-browser-download) for the source, cache, integrity and deployment behavior.
