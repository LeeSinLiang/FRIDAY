...
The room index and preview deck read the existing build-provided room catalogue. `shared/public-rooms.json` lists Haussmann apartment first, followed by the studio, London skyscraper and Cg Arch living room. Haussmann is initially selected and starts downloading in the background when its local visual asset is missing. The large view is the existing room preview, including any editor controls captured in that image; it is not a generated architectural substitute. Other unpackaged rooms remain disabled. Haussmann has a verified browser-download fallback, real progress and retry. A failed preview displays a fallback without changing room availability.
