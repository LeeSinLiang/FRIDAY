...
- **Room: open `/?room=cg-arch-interior`. Always the full URL, never the bare address.** The editor's default room is the Haussmann splat apartment (Sin's choice, in `SplatEditor.tsx`), so the bare URL opens a different room from the one every number in this document was measured in. There is no environment variable for this any more: `VITE_DEFAULT_ROOM_ID` is not read by any code, and setting it does nothing. `?roomId=cg-arch-interior` works too. Bookmark the full URL on the presenting machine.
...
1. **Search.** In the catalogue panel (bottom left) type `an armchair`, press **Find**. The chip reads `armchair`, and under the list: **"1,006 matches in the catalogue · 1 ready in 3D"**. Search counts all twelve thousand listings and shows the ones you can actually place as a real model; say that, it is the Elastic story and the honest one. (The "ready in 3D" number grows as models land; if there are ever too few, `SEARCH_RESULTS_REQUIRE_MODEL=boost` in `.env` shows everything with those first; that mode is PR #65.)
2. **Hover the HERRÅKRA armchair.** The floor lights up green with everywhere its centre can go.
...
8. **Price, as its own beat.** Type `armchairs under $400`. Chips `armchair` · `under $400`; the panel says **"392 matches in the catalogue · 1 ready in 3D"**. A listing whose price we do not know never matches a price filter, is counted in no price band, and its card says "price unavailable", never "$0". Re-walk this beat once unpriced listings are in the catalogue.
