...
5. **Or type it.** `a reading chair by the window, under $400, 4 feet from any wall` → chips appear, the catalogue narrows, and the lit floor narrows with it. (The fixture room has no windows yet, so "by the window" is listed under the panel as a clause it could not use. That is the honest behaviour; say so or pick a sentence without a window.)
