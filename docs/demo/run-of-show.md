...
8. **Price, as its own beat.** Type `armchairs under $400`. Chips `armchair` · `under $400`; the panel says **"392 matches in the catalogue · 1 ready in 3D"**. Every listing on `main` today has a real price. (When listings with no known price arrive, they must never match a price filter; that rule belongs to the PR that brings them in, and this beat should be re-walked then.)
