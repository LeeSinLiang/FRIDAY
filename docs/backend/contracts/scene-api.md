...
### The same principle in the PlayCanvas editor (2026-09-20)

The PlayCanvas editor's session (`frontend/src/scene/useRoomSession.ts`) is **server-authoritative**: its snapshot only ever holds what the server accepted, and on a 5xx or a lost response it goes `offline` and keeps the request, same `commandId`, for `retry()`. Left alone that would refuse a catalogue placement over a storage hiccup, which is what this section forbids.

Catalogue placements therefore go through `frontend/src/catalogue/quietSave.ts`, driven from `SplatCatalogueLayer.tsx`, without editing the session: the item appears at once as a true-size box in its listing colour (`scene/playcanvas/pendingGhost.ts`) and counts as standing for the region solver; an `offline` result calls the session's own `retry()` on the 0.5 s, 1 s, 2 s, 4 s, 8 s backoff until the server accepts; only a definite refusal (4xx or conflict, i.e. the session is no longer `offline`) removes it.

**Known lag, by design for now.** Everything the editor itself draws from the snapshot trails the floor by the retry time: the Objects list and "In your layout" count do not include the item, the header reads *Disconnected* with a *Retry* button, and the editor is locked, so nothing else can be placed until the save lands. The item's real model also replaces the box only then. Drags and removals inside the editor do not go through this path and are still refused outright on a 5xx. Both are follow-ups for the editor's owner, recorded in `TODO_SIN.md`.
