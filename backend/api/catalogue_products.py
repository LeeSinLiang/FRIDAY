...
SCENE_KINDS = {'sofa', 'table', 'chair'}
MAX_TEXT = 512


def well_formed(carried, product_id):
    """Every field of a carried product has the type and range the editor's own parser demands.

    Anything the server stores, the browser must be able to load again; a product that passes here
    but fails validInlineProduct in frontend/src/scene/products.ts would lock that session's editor.
    """
    if not isinstance(carried, dict) or not PRODUCT_KEYS <= set(carried) or set(carried) - PRODUCT_KEYS - OPTIONAL_PRODUCT_KEYS:
        return False
    text = lambda value: isinstance(value, str) and 0 < len(value.strip()) and len(value) <= MAX_TEXT
