...
from .scene_service import SceneError, apply_scene_commands, room_presets, save_scene, scene_for_session, serialize
...
def room_id(request):
    explicit = request.query_params.get('roomId')
    if explicit is not None:
        return explicit
    preset = request.COOKIES.get(ROOM_COOKIE)
    return preset if preset in room_presets() else 'demo-room'
...
def scene_commands(request):
    ...
    return execute(lambda: apply_scene_commands(session_key(request), request.data['baseRevision'], request.data['commandId'], request.data['commands'], room_id(request)))
...
def scene_placement(request):
    ...
    return execute(lambda: attempt_placement(session_key(request), request.data, room_id(request)), explicit=True)
...
def captures(request):
    ...
    return execute(lambda: enqueue_capture(session_key(request), request.data, room_id(request)), status=202)
...
def capture_claim(request):
    ...
    return execute(lambda: claim_capture(session_key(request), room_id(request)))
...
def capture_complete(request, capture_id):
    ...
    return execute(lambda: complete_capture(session_key(request), capture_id, request.data, room_id(request)))
...
def capture_detail(request, capture_id):
    ...
    return execute(lambda: metadata(get_capture(session_key(request), capture_id, room_id(request))))
...
def capture_image(request, capture_id):
    ...
    try:
        job = get_capture(session_key(request), capture_id, room_id(request))
        if job.status != 'ready':
