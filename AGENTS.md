...
- Prefer the running board's HTTP API or canvas actions for edits; they update its cache and notify open tabs. Read the current board first and make targeted task changes. If direct JSON edits are necessary, stop this checkout's board server first, re-read and preserve the latest data, then restart and verify it. A running process can overwrite external JSON edits from its cached copy.
