...
- **Never report a status without quoting the number you read.** "Backend OK" is a claim; `Ran 206 tests in 8.2s` / `OK (skipped=2)` pasted from the output is a reading. `main` was once reported green by an agent that had not looked at the backend result, and it was red. No shell option fixes stating a result you did not read. If you did not read it, say you did not.
- **When you report, name the command and quote its output line.** Not "full suite green" but "`python manage.py test` → Ran 206 tests in 8.076s / OK". "Full suite", "green" and "pushed" are summaries, and a summary is where the gap between what you did and what you claimed hides. Four failures here had that one shape: a piped push, a result nobody read, a "full suite" that skipped three apps, and a test count written down before the run finished.
- **CI runs all of it on every pull request** (`.github/workflows/ci.yml`): the unlabelled backend suite, `npm test` and `npm run build`, with no secrets. It is advisory, not a required check, so it cannot block an emergency fix. A red X on your PR means do not press Merge; it does not replace running the suite on fresh `main` after you merge, because CI tests the PR against the `main` it was cut from.
- **The full backend suite is `manage.py test` with no app labels** (206 tests on 2026-09-20; keys blanked as in README.md). A label list such as `test api catalogue` runs 170 and silently skips `accounts`, `checkout` and `visa`, and will skip any app added later. The frontend had the same gap: `npm test` did not run the two auth test files until 2026-09-20. It does now, and a test fails if any test file on disk is unreachable from it. If the count you read drops, find out where the tests went before doing anything else.
...
## Release the merged source to Vercel

Merging or pushing GitHub `main` does **not** currently deploy FRIDAY. The two existing Vercel projects have no working Git connection ([issue #74](https://github.com/LeeSinLiang/hackmit2026/issues/74)). Until the repository owner connects both projects and a PR Preview plus a `main` Production trigger are verified, release them manually as a pair. See the [deployment guide](docs/frontend/room-cart-deployment.md) for environment and migration details.

1. Complete the PR/CI merge and the fresh-`main` setup, full backend suite, `npm test`, and frontend build above. Deploy from that **clean, tested `main` source**, not an older feature worktree. If another worktree owns local `main`, use a fresh worktree at the fetched `origin/main`; do not disturb its checkout. Record `git rev-parse HEAD` and `python3 scripts/source_identity.py`. The latter hashes runtime source contents, not the commit ID.
2. Use the existing `williamxu070s-projects` Vercel team and existing projects: `friday-hackmit-api` (project root `backend`) and `friday-hackmit` (project root `frontend`). Link each from the **repository root** using `npx --yes vercel link --yes --team williamxu070s-projects --project <project-name>` and verify `.vercel/project.json` names the intended project before deploying. The second link replaces the local project selection. Do not create duplicate Vercel projects or deploy just a copied subdirectory; both builds need shared repository files. Vercel may create ignored local env files or propose a `.gitignore` edit: review `git status` and keep those generated changes out of the release commit.
3. Check that the API Production environment has its existing database, Django secret/host/CSRF, catalogue, OpenAI/Deepgram and other required settings. Check that frontend Production `FRIDAY_API_ORIGIN` is the stable `https://friday-hackmit-api.vercel.app` origin, not a previous immutable deployment. Keep Preview and Production databases and secrets separate. Never print, commit, or pass secret values in a command log. Run required schema migrations with `scripts/vercel_release.py` as documented before releasing code that needs them.
4. Deploy the API first from the repository root: `npx --yes vercel deploy --prod --yes --scope williamxu070s-projects --env FRIDAY_BUILD_ID=<source-identity>`. Wait for `vercel inspect <deployment-url>` to report `READY`. Then relink the root to `friday-hackmit` and deploy the frontend with `npx --yes vercel deploy --prod --yes --scope williamxu070s-projects --build-env VITE_FRIDAY_BUILD_ID=<same-source-identity>`. Confirm that project's deployment is also `READY`. Replace each placeholder with the exact value computed in step 1; use the same value for both. Record the two deployment IDs and stable aliases.
5. Verify the **stable public workflow** after promotion: GET `https://friday-hackmit-api.vercel.app/api/health/` and `https://friday-hackmit.vercel.app/api/health/` (Django health does not accept HEAD), check both `X-Friday-Build` headers, and read the frontend's live DOM `data-build` in a browser. All three must equal the computed source identity. Open `/rooms`, the affected `/room/...` route, its assets and the changed user action; confirm save/reload when relevant without overwriting an existing guest scene. An immutable Vercel hostname can fail CSRF because it is outside the configured trusted frontend origin, so it is not a substitute for the stable-origin write test. If any identity, route, action or deployment status differs, report the release incomplete and repair the actual alias/runtime before claiming delivery.

The repository owner can later connect **both existing** Vercel projects to `LeeSinLiang/hackmit2026`, preserve their `frontend`/`backend` roots, and select `main` as Production Branch. A Git connection is proven only when a PR creates Preview deployments and a subsequent `main` merge creates two Ready Production deployments on the existing projects. Keep the manual sequence until that trigger and the runtime identities are verified.
