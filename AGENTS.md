...
- **The full backend suite is `manage.py test` with no app labels** (206 tests on 2026-09-20; keys blanked as in README.md). A label list such as `test api catalogue` runs 170 and silently skips `accounts`, `checkout` and `visa`, and will skip any app added later. If the count you read drops, find out where the tests went before doing anything else.
