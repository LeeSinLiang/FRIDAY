...
export function parkPending(store: PendingStore | undefined, roomId: string, pending: Pending): void {
  try { store?.setItem(key(roomId), JSON.stringify(pending)); } catch { /* private mode or a full quota: the edit is still in memory */ }
}
