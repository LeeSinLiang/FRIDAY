...
const cueFirst = /^do\s+it\s+now\b[\s,.:;!\-]*/i;
const cueLast = /[\s,.:;!\-]+do\s+it\s+now[.!?]*$/i;
...
export function designerInstruction(text: string, priorDraft = ""): string | null {
  ...
  const hasCue = cueFirst.test(spoken) || cueLast.test(spoken);
  let value = spoken.replace(cueFirst, "").replace(cueLast, "").trim();
  if (!value && hasCue) value = priorDraft.trim().replace(cueFirst, "").replace(cueLast, "").trim();
