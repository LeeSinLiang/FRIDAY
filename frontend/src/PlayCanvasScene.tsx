...
export default function PlayCanvasScene(props:Props) {
  ...
  return <><canvas ref={canvas} className="splat-canvas" aria-label="Interactive room. Drag to look; choose Walk for WASD movement." tabIndex={0}/><PlayCanvasCapture runtime={runtime} enabled={ready && props.captureEnabled !== false}/>{perfEnabled && runtime && <PerformancePanel runtime={runtime} startedAt={startedAt.current}/>}</>;
}
